export class VCManager {
    ...
    constructor() {
        // We'll initialize elements later when they're available
    }
    ...
    public initialize() {
        ...
        if (!this.startButton || !this.endButton || !this.statusDiv) {
            console.error("Voice chat UI elements not found. Make sure they exist in the DOM.");
            return false;
        }
    ...
    public startCall() {
        try {
            if (!this.startButton || !this.endButton) {
                console.error("Voice chat UI elements not found. Did you call initialize()?");
                return;
            }
            ...
            const wsUrl = `wss://fluffy-cod-9x69r9ggx573pvwx-12345.app.github.dev/ws/vc/${clientId}`;

            this.updateStatus(`Connecting to WebSocket: ${wsUrl}`);
